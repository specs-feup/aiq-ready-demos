 ...
 * 4. Add new `t_*` variable under "Perform computation in several implementations".
...
//#include <assert.h> // assert() --> disabled by OpenTitan; CHECK() alternative not provided by X-HEEP
//#include "csr.h" // CSR_*

/** Customizable macros **/

// Define matrix dimensions for matrix multiplication operation
#define IV_LEN 32 //32 //  inpupt vector length | matrix width  (assuming MxV)  // FIXME: Temporarily changed to 32
#define OV_LEN 16 // outpupt vector length | matrix height (assuming MxV)  // FIXME: Temporarily changed to 16
// Note: currently IV_LEN must be a multiple of 4 in order to write word by word.
// Note: IV_LEN and OV_LEN must fit in the accelerator's memories.
...
#define CR_0_CONFIG_WORD (CR_0_INPUT_BITS(IV_BIT) | CR_0_OUTPUT_BITS(OV_BIT) | CR_0_CLR_ON_RD)
...
#if (IV_BIT < 32 && IV_LEN % (32/IV_BIT) != 0) || (OV_BIT < 32 && OV_LEN % (32/OV_BIT) != 0)
...
#define V(n)   if (VERBOSITY >= (n))

/** CPU memory access functions **/

/* Replacement for memcpy() but using volatile.
 * (Note that using memcpy() directly may not work since it's not intended for volatile data.)
 * 
 * Note: src, dst, and bytes MUST be aligned to 4-byte (32-bit) word boundaries.
 */
static void volatile_memcpy(volatile void *restrict dst, volatile const void *restrict src, size_t bytes) {
    V(3) printf("Memcpy %p->%p (%d B)\r\n", (void *)src, (void *)dst, (int)bytes);
    
    // Ensure that everything is aligned to 4-byte (32-bit) words
    //assert((uintptr_t)dst % 4 == 0); // assert() disabled by OpenTitan; CHECK() alternative not provided by X-HEEP
    //assert((uintptr_t)src % 4 == 0);
    //assert(bytes % 4 == 0);
    
    volatile uint32_t *dst_w = dst, *src_w = src;
    size_t words = bytes / sizeof *dst_w;
    for (size_t i = 0; i < words; i++) dst_w[i] = src_w[i];
}

/* Same but with 8x loop unroll */
static void volatile_memcpy_unroll(volatile void *restrict dst, volatile const void *restrict src, size_t bytes) {
    V(3) printf("Memcpy %p->%p (%d B)\r\n", (void *)src, (void *)dst, (int)bytes);
    ...
    for (size_t i = (words/8)*8; i < words; i++) dst_w[i] = src_w[i];
}
...
static int custom_rand_range(int min, int max) {
    ...
    int64_t x = ((uint64_t)custom_rand_next * range) >> 32; // get MOST sig bits of LCG
    return x + min;
}
...
static ov_elem_t out_vec_custom_instr_unroll[OV_LEN]; // Custom instr, CPU moves data (8x unroll)
...
static inline void mac_4b_custom (signed int a, signed int b, signed int c, signed int d) {
    asm volatile (
        ".insn r 0b0001011, 0x00, 0x0, x0, %[RS1], %[RS2]\n" // 4b config - MAC - custom0 (Instruction 1)
        ".insn r 0b0001011, 0x00, 0x0, x0, %[RS3], %[RS4]" // 4b config - MAC - custom0 (Instruction 2)
        :
...
// .insn r opcode6, func3, func7, rd, rs1, rs2
static inline void mac_8b_custom (signed int a, signed int b, signed int c, signed int d) {
    asm volatile (
        ".insn r 0b0001011, 0x02, 0x0, x0, %[RS1], %[RS2]\n" // 8b config - MAC - custom0 (Instruction 1)
        ".insn r 0b0001011, 0x02, 0x0, x0, %[RS3], %[RS4]" // 8b config - MAC - custom0 (Instruction 2)
        :
        : [RS1] "r" (a), [RS2] "r" (b), [RS3] "r" (c), [RS4] "r" (d)
    );
}
...
// .insn r opcode6, func3, func7, rd, rs1, rs2
static inline void mac_16b_custom (signed int a, signed int b, signed int c, signed int d) {
    asm volatile (
        ".insn r 0b0001011, 0x04, 0x0, x0, %[RS1], %[RS2]\n" // 16b config - MAC - custom0 (Instruction 1)
        ".insn r 0b0001011, 0x04, 0x0, x0, %[RS3], %[RS4]" // 16b config - MAC - custom0 (Instruction 2)
        :
        : [RS1] "r" (a), [RS2] "r" (b), [RS3] "r" (c), [RS4] "r" (d)
    );
}
...
// .insn r opcode6, func3, func7, rd, rs1, rs2
static inline void mac_32b_custom (signed int a, signed int b, signed int c, signed int d) {
    asm volatile (
        ".insn r 0b0001011, 0x06, 0x0, x0, %[RS1], %[RS2]\n" // 32b config - MAC - custom0 (Instruction 1)
        ".insn r 0b0001011, 0x06, 0x0, x0, %[RS3], %[RS4]" // 32b config - MAC - custom0 (Instruction 2)
        :
        : [RS1] "r" (a), [RS2] "r" (b), [RS3] "r" (c), [RS4] "r" (d)
    );
}
...
static inline signed int read_clear_custom () {
    ...
    asm volatile (
        ".insn r 0b0001011, 0x07, 0x0, %[RD], x0, x0" // 32b config - R+CLR - custom0 
        : [RD] "=r" (result)
    );
...
/* Compute using the custom instructions for matrix-vector multiplication with loop unrolling */
static inline void MxV_custom_8b_unroll(void) {
    for (int i = 0; i < OV_LEN; i++) {
        ...
        for (int j = 0; j < (PACKED_LEN / (2*8)) * 2*8; j += 2*8) {
            mac_8b_custom(packed_in_mat[i][j +   0], packed_in_vec[j +   0], packed_in_mat[i][j +     1], packed_in_vec[j +     1]);
            mac_8b_custom(packed_in_mat[i][j + 2*1], packed_in_vec[j + 2*1], packed_in_mat[i][j + 2*1+1], packed_in_vec[j + 2*1+1]);
            mac_8b_custom(packed_in_mat[i][j + 2*2], packed_in_vec[j + 2*2], packed_in_mat[i][j + 2*2+1], packed_in_vec[j + 2*2+1]);
            mac_8b_custom(packed_in_mat[i][j + 2*3], packed_in_vec[j + 2*3], packed_in_mat[i][j + 2*3+1], packed_in_vec[j + 2*3+1]);
            mac_8b_custom(packed_in_mat[i][j + 2*4], packed_in_vec[j + 2*4], packed_in_mat[i][j + 2*4+1], packed_in_vec[j + 2*4+1]);
            mac_8b_custom(packed_in_mat[i][j + 2*5], packed_in_vec[j + 2*5], packed_in_mat[i][j + 2*5+1], packed_in_vec[j + 2*5+1]);
            mac_8b_custom(packed_in_mat[i][j + 2*6], packed_in_vec[j + 2*6], packed_in_mat[i][j + 2*6+1], packed_in_vec[j + 2*6+1]);
            mac_8b_custom(packed_in_mat[i][j + 2*7], packed_in_vec[j + 2*7], packed_in_mat[i][j + 2*7+1], packed_in_vec[j + 2*7+1]);
        }
...
static int compare(ov_elem_t obtained[], ov_elem_t expected[], int n_elem) {
    ...
    for (int i=0; i<n_elem; i++) {
        if (obtained[i] == expected[i]) {
            good++;
            V(3) printf(
                "... good:     OV[%d]: got %d (0x%X), expected %d (0x%X)\r\n",
                i,
                (int)obtained[i], (int)obtained[i],
                (int)expected[i], (int)expected[i]
            );
        } else {
            bad++;
            V(1) printf(
                "*** MISMATCH: OV[%d]: got %d (0x%X), expected %d (0x%X)\r\n",
                i,
                (int)obtained[i], (int)obtained[i],
                (int)expected[i], (int)expected[i]
            );
    ...
    V(0) printf("Finished: %d correct, %d wrong.\r\n", good, bad);
    return bad;
}
...
int main(void) {
    ...
    V(1) printf("Initializing timer\r\n");
    //CSR_CLEAR_BITS(CSR_REG_MCOUNTINHIBIT, 0x1); // Enable mcycle CSR
    //CSR_WRITE(CSR_REG_MCYCLE, 0); // Initialize mcycle to 0
    
    /** Initialize problem data **/
    
    V(1) printf("Initializing data\r\n");
    V(2) printf("- bias[*]\r\n");
    for (int i=0; i<OV_LEN; i++) bias[i] = 0; // could be randomized as well
    V(2) printf("- in_vec[*]\r\n");
    for (int j=0; j<IV_LEN; j++) in_vec[j] = custom_rand_range(-(1<<(IV_BIT-1)), (1<<(IV_BIT-1))-1);
    for (int i=0; i<OV_LEN; i++) {
        V(2) printf("- in_mat[%d][*]\r\n", i);
        for (int j=0; j<IV_LEN; j++) {
            in_mat[i][j] = custom_rand_range(-(1<<(IM_BIT-1)), (1<<(IM_BIT-1))-1);
        }
    }
    
    /** Pack data **/
    V(1) printf("Packing data\r\n");
    pack_data();
    ...
    V(1) printf("CPU computation (unrolling)\r\n");
    //CSR_READ(CSR_REG_MCYCLE, &t0); // start timer
    MxV_sw_unroll();
    //CSR_READ(CSR_REG_MCYCLE, &t1); // stop timer
    //t_sw_unroll = t1 - t0;
    ...
    V(1) printf("Custom Instructions (8-bit)\r\n");
    //CSR_READ(CSR_REG_MCYCLE, &t0); // start timer
    MxV_custom_8b();
    //CSR_READ(CSR_REG_MCYCLE, &t1); // stop timer
    ...
    // Compute using custom instructions (8-bit)
    V(1) printf("Custom Instructions (8-bit) (unrolling)\r\n");
    //CSR_READ(CSR_REG_MCYCLE, &t0); // start timer
    ...
    V(1) printf("Checking result\r\n");
    
    V(0) printf("CPU (+ 8x loop unrolling)\r\n");
    int bad_sw_unroll = compare(out_vec_sw_unroll, out_vec_sw, OV_LEN);
        //^ need to use out_vec_sw_unroll; otherwise call to MxV_sw_unroll() gets optimized out
    V(0) printf("TEST %s\r\n", (bad_sw_unroll) ? "FAILED" : "PASSED");
    
    V(0) printf("Custom Instructions (8-bit)\r\n");
    int bad_custom_8b = compare(out_vec_custom_instr, out_vec_sw, OV_LEN);
    V(0) printf("TEST %s\r\n", (bad_custom_8b) ? "FAILED" : "PASSED");

    V(0) printf("Custom Instructions (8-bit) (unrolling)\r\n");
    int bad_custom_8b_unroll = compare(out_vec_custom_instr_unroll, out_vec_sw, OV_LEN);
    V(0) printf("TEST %s\r\n", (bad_custom_8b_unroll) ? "FAILED" : "PASSED");
    
    /** Print times **/
    
    V(1) printf("Printing timing result\r\n");
    //V(0) printf("- CPU:            %9u clock cycles\r\n", t_sw);
    //V(0) printf("- CPU (unrolled): %9u clock cycles\r\n", t_sw_unroll);
    //V(0) printf("- Custom 8-bit:   %9u clock cycles\r\n", t_hw_custom_8b);
    //V(0) printf("- Custom 8-bit (unrolled):   %9u clock cycles\r\n", t_hw_custom_8b_unroll);
    if (bad_sw_unroll || bad_custom_8b || bad_custom_8b_unroll) {
        return 1; // Not all tests passed
    } 
